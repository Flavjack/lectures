...
```{r}
snpdat <- calc_he(snpdat, facets = "pop") # running the calculation
get.snpR.stats(snpdat, facets = "pop", stats = "he")$weighted.means # retrieving the results
```

```{r}
get.snpR.stats(snpdat, facets = "pop", stats = c("he", "ho"))$weighted.means
```
...
```{r}
pca <- plot_clusters(snpdat, facets = c("pop"))
pca
```
...
plot_pairwise_fst_heatmap(snpdat, facets = "pop")
```

## Population structure

```{r}
plot_structure(snpdat, facets = "pop", method = "snapclust", k = 2:10)

plot_structure(snpdat, facets = "pop", method = "snapclust", k = 4)
```
