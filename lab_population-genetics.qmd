...
```{r}
rubiVCF <- read.vcfR("datasets/prubi_gbs.vcf.gz")
rubiVCF
...
```{r}
glrubi <- snpR::convert_vcfR("datasets/prubi_gbs.vcf.gz")
glrubi
...
stat <- get.snpR.stats(glrubi, facets = "pop", stats = "single")
stat %>% kable()
...
```{r}
gd <- snpR::calc_genetic_distances(glrubi
                                   , facets = c("sampID")
                                   , method = "Edwards")
dat <- get.snpR.stats(gd
                      , facets = "sampID"
...
```{r}
snpR::plot_structure(glrubi, facet = c("pop")
                     , k = 2:10
                     , clumpp = FALSE
                     , method = "snapclust")

snpR::plot_structure(glrubi, facet = c("pop"), k = c(3))
```
